...
export const SearchBox: React.FC<SearchBoxProps> = ({ onSearch, onFiltersChange, maxHistoryItems = 5 }) => {
  ...
  const searchSuggestions = [
    { type: 'member' as const, icon: <PersonIcon fontSize="small" />, label: '@担当者', prefix: '@' },
    { type: 'tag' as const, icon: <TagIcon fontSize="small" />, label: '#タグ', prefix: '#' },
    { type: 'progress' as const, icon: <ProgressIcon fontSize="small" />, label: '%進捗', prefix: '%' },
    { type: 'priority' as const, icon: <PriorityIcon fontSize="small" />, label: '!優先度', prefix: '!' },
    { type: 'status' as const, icon: <StatusIcon fontSize="small" />, label: '$ステータス', prefix: '$' },
    { type: 'capacity' as const, icon: <CapacityIcon fontSize="small" />, label: '^稼働率', prefix: '^' },
  ];
